...
def test_decorator_strict_propagates():
    ...
    mock_handler = MagicMock(spec=DataHandler)
    mock_handler.fetch.side_effect = OSError("File not found")
...
def test_decorator_kwargs_passthrough():
    ...
    scenario._data = Register[NumKey]()
...
def test_scenario_integration():
    ...
    TestSalesVolume = NumKey(100, "test_sales", "test_sales", float)
    TestPrice = NumKey(101, "test_price", "test_price", float)
...
def test_dump_deletes_existing_version_records():
    ...
    scenario.dump(mock_session, set(), (Dimension("Test", "", ""),), fact=False)
...
def test_dump_uses_fact_table_when_fact_true():
    ...
    with patch("or_scenario.orm.generate_fact_table") as mock_fact_table:
        ...
        scenario.dump(mock_session, set(), (TestDimension,), fact=True)
...
def test_dump_inserts_records_for_all_indexes():
    ...
    scenario.dump(mock_session, {SalesVolume}, (TestDimension,), fact=False)
